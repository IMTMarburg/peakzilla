...
class TagContainer:
    ...
    def __call__(self, bed_file, is_pe):
        ...
        if not is_pe:
            if bed_file.endswith('bam'):
                self.load_bam(bed_file)
            else:
                self.load_bedpe(bed_file)
        else:
            if bed_file.endswith('bam'):
                self.load_bam_pe(bed_file)
            else:
    ...
    def load_bam(self, bam_file):
        ...
        for read in bam.fetch():
            ...
            if chrom is not None:
                if read.is_reverse:
                    fiveprime = read.reference_end #  already + 1
                    strand = '-'
                else:
                    fiveprime = read.reference_start + 1  #bam is 0 based, peakzilla apperantly works on ones
                    strand = '+'
                self.add_tag(chrom, strand, fiveprime)

    def load_bam_pe(self, bam_file):
        import pysam
        bam = pysam.Samfile(bam_file)
        uids = set()
        fragment_lengths = []
            ...
            if read.is_read1:
                ...
                if chrom is not None:
                    ...
                    if read.next_reference_name == chrom:
                        end = read.next_reference_start + read.query_alignment_length # not exact but saves a ton of seeking
                        uid = chrom, start, end
                        if not uid in uids:
                            uids.add(uid)
